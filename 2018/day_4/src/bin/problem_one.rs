...
fn main() {
    ...
    // Lastly, getting the guard that sleeps most, and then finding their most slept minute
    let (&sleepy, sleep_times) = times
        .iter()
        .max_by_key(|(_, times)| {
            // Don't care about ID here, just calc total sleep time
            let mut total = 0;
            for &(start, end) in times.iter() {
                total += end - start;
            }
            total
        })
    ...
    let most_slept = (0..60)
        .max_by_key(|&t| {
            // Getting times slept during this minute
            let mut total = 0;
    ...
    println!("{} * {} = {}", sleepy, most_slept, sleepy * most_slept);
}
